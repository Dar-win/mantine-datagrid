...
function MDataGrid<T>({
  ...
  useEffect(() => {
    ...
    if (colParams && limitParam && offsetParam) {
      const cols: { s: string[]; r: string[]; g: string[] } = JSON.parse(
        colParams,
      );
...
export function DataGrid<T>({
  ...
  return (
    <DataGridProvider>
      {loading &&
        <LoadingTable tableName={tableName} columnsDef={columns} />}
      <MDataGrid
        columns={columns}
        data={data}
        loading={loading}
